			...
			if t.Get("type").String() == "function" {
				...
				if fn.Exists() && fn.IsObject() {
					...
					if prm := fn.Get("parameters"); prm.Exists() {
						...
						fnJSON, _ = sjson.SetRaw(fnJSON, "parameters", cleaned)
						// Drop parametersJsonSchema to avoid duplication
